...
describe('Full end-to-end chain: library → kosztorys → accept → umowy + zamówienia → realizacja', () => {
  ...
  beforeAll(async () => {
    ...
    const typMontazProfili = await createTypRobocizny(orgId, { nazwa: 'Montaż profili' });
    const typSzpachlowanie = await createTypRobocizny(orgId, { nazwa: 'Szpachlowanie' });
    const typMalowanie = await createTypRobocizny(orgId, { nazwa: 'Malowanie' });

    await createStawkaPodwykonawcy({ podwykonawca_id: kowalski.id, typ_robocizny_id: typMontazProfili.id, stawka: 15.0 });
    await createStawkaPodwykonawcy({ podwykonawca_id: kowalski.id, typ_robocizny_id: typSzpachlowanie.id, stawka: 12.0 });
    await createStawkaPodwykonawcy({ podwykonawca_id: malarz.id, typ_robocizny_id: typMalowanie.id, stawka: 20.0 });

    const pSciana = await createPozycjaBiblioteka(orgId, {
      kategoria_id: podkat01.id, kod: 'BUD.01.01.001', nazwa: 'Ściana GK C50', jednostka: 'm2', typ: 'komplet',
    });
    const pMalowanie = await createPozycjaBiblioteka(orgId, {
      kategoria_id: podkat02.id, kod: 'BUD.01.02.001', nazwa: 'Malowanie ścian', jednostka: 'm2', typ: 'komplet',
    });

    // Biblioteka składowe — Ściana GK (robocizna cena = stawka rate)
    await createBibliotekaSkladowaM({ pozycja_biblioteka_id: pSciana.id, lp: 1, produkt_id: profil.id, dostawca_id: bricoman.id, norma_domyslna: 0.9, jednostka: 'mb' });
    await createBibliotekaSkladowaM({ pozycja_biblioteka_id: pSciana.id, lp: 2, produkt_id: plyta.id, dostawca_id: bricoman.id, norma_domyslna: 1.1, jednostka: 'm2' });
    await createBibliotekaSkladowaR({ pozycja_biblioteka_id: pSciana.id, lp: 1, typ_robocizny_id: typMontazProfili.id, podwykonawca_id: kowalski.id, cena: 15.0 });
    await createBibliotekaSkladowaR({ pozycja_biblioteka_id: pSciana.id, lp: 2, typ_robocizny_id: typSzpachlowanie.id, podwykonawca_id: kowalski.id, cena: 12.0 });

    // Biblioteka składowe — Malowanie
    await createBibliotekaSkladowaM({ pozycja_biblioteka_id: pMalowanie.id, lp: 1, produkt_id: farba.id, dostawca_id: sig.id, norma_domyslna: 0.15, jednostka: 'l' });
    await createBibliotekaSkladowaR({ pozycja_biblioteka_id: pMalowanie.id, lp: 1, typ_robocizny_id: typMalowanie.id, podwykonawca_id: malarz.id, cena: 20.0 });
    ...
    const kpSciana = await createKosztorysPozycja(orgId, {
      rewizja_id: rewizjaId, pozycja_biblioteka_id: pSciana.id,
      lp: 1, nazwa: 'Ściana GK C50', ilosc: 320, jednostka: 'm2', narzut_percent: 30,
    });
    kpScianaId = kpSciana.id;

    // robocizna cena = stawka * norma (per-unit labor cost contribution)
    await createKosztorysSkladowaM({ kosztorys_pozycja_id: kpScianaId, lp: 1, produkt_id: profil.id, dostawca_id: bricoman.id, cena: 8.5, norma: 0.9 });
    await createKosztorysSkladowaM({ kosztorys_pozycja_id: kpScianaId, lp: 2, produkt_id: plyta.id, dostawca_id: bricoman.id, cena: 22.0, norma: 1.1 });
    await createKosztorysSkladowaR({ kosztorys_pozycja_id: kpScianaId, lp: 1, typ_robocizny_id: typMontazProfili.id, podwykonawca_id: kowalski.id, cena: 4.5 }); // 15*0.3
    await createKosztorysSkladowaR({ kosztorys_pozycja_id: kpScianaId, lp: 2, typ_robocizny_id: typSzpachlowanie.id, podwykonawca_id: kowalski.id, cena: 2.4 }); // 12*0.2

    // Kosztorys — Malowanie (500 m2, 25%)
    const kpMal = await createKosztorysPozycja(orgId, {
      rewizja_id: rewizjaId, pozycja_biblioteka_id: pMalowanie.id,
      lp: 2, nazwa: 'Malowanie ścian', ilosc: 500, jednostka: 'm2', narzut_percent: 25,
    });
    kpMalowanieId = kpMal.id;

    await createKosztorysSkladowaM({ kosztorys_pozycja_id: kpMalowanieId, lp: 1, produkt_id: farba.id, dostawca_id: sig.id, cena: 35.0, norma: 0.15 });
    await createKosztorysSkladowaR({ kosztorys_pozycja_id: kpMalowanieId, lp: 1, typ_robocizny_id: typMalowanie.id, podwykonawca_id: malarz.id, cena: 2.0 }); // 20*0.1
  ...
  it('kosztorys totals are correct', async () => {
    ...
    // r_jedn = SUM(cena) = 4.50 + 2.40 = 6.90
  ...
  it('zamowienia generated correctly per dostawca', async () => {
    ...
    expect(Number(bricoPoz![0].ilosc_zamowiona)).toBeCloseTo(288, 0);
    expect(Number(bricoPoz![1].ilosc_zamowiona)).toBeCloseTo(352, 0);

    const sigZam = zamowienia!.find((z) => (z.dostawcy as unknown as { nazwa: string }).nazwa === 'SIG');
    const { data: sigPoz } = await supabase.from('zamowienie_pozycje').select('ilosc_zamowiona').eq('zamowienie_id', sigZam!.id);
  ...
  it('budget consistency across layers', async () => {
    ...
    // Ściana: r_jednostkowy × ilosc = 6.90 × 320 = 2208
  ...
  it('realizacja tracks spending against budget', async () => {
    ...
    await supabase.from('realizacja_wpisy').insert({
      organization_id: orgId, projekt_id: projektId, typ: 'material',
      kwota_netto: 5000, numer_faktury: 'FV/2026/F001', zamowienie_id: zamowienia![0].id,
    });
    await supabase.from('realizacja_wpisy').insert({
      organization_id: orgId, projekt_id: projektId, typ: 'robocizna',
      kwota_netto: 2000, numer_faktury: 'FV/2026/F002', umowa_id: umowy![0].id,
    });

    const { data: wpisy } = await supabase.from('realizacja_wpisy').select('kwota_netto').eq('projekt_id', projektId);
